...
export default function WarmChapterSection() {
  ...
  const displayCards = useMemo(() => {
    // 1. If categories from DB/store exist, use categories marked as warm chapter and active
    ...
    // 2. If warmChapters exist from API endpoint and active
    ...
    // 3. Fallback to active categories from store if none specifically marked
    if (categories && categories.length > 0) {
  ...
  // 4-second Auto-scroll interval with pause on hover
  useEffect(() => {
  ...
  return (
    <section 
      ...
      className="py-16 md:py-20 bg-white border-b border-gray-100 relative"
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
    >
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header with red underline styling */}
          ...
          {maxIndex > 0 && (
            <>
              <button
                ...
                className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 sm:-translate-x-4 z-20 w-10 h-10 sm:w-12 sm:h-12 bg-black/80 hover:bg-black text-white flex items-center justify-center shadow-lg transition-all opacity-80 hover:opacity-100 hover:scale-105 border border-white/20"
              >
                <ChevronLeft size={22} />
              </button>
              <button
                ...
                className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 sm:translate-x-4 z-20 w-10 h-10 sm:w-12 sm:h-12 bg-black/80 hover:bg-black text-white flex items-center justify-center shadow-lg transition-all opacity-80 hover:opacity-100 hover:scale-105 border border-white/20"
              >
            ...
            >
              {displayCards.map((chapter, idx) => (
                ...
                >
                  ...
                  >
                    {/* Card with 9/16 portrait ratio */}
                    <div className="relative aspect-[9/16] overflow-hidden bg-zinc-900 shadow-md group-hover:shadow-2xl transition-all duration-300">
                      ...
                      {/* Bottom dark gradient overlay for crystal clear contrast */}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/35 to-transparent flex flex-col justify-end p-5 sm:p-6 transition-all duration-300">
                        {chapter.subtitle && (
