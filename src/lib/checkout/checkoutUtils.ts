...
export async function sendOTPViaNginx(phone: string, otp: string): Promise<boolean> {
  try {
    ...
    console.log(`[OTP] Sent to ${phone}: ${otp}`); // Console fallback for dev
    return true;
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'send') {
      ...
      await saveOTP(phone, generatedOTP);
      
      const sent = await sendOTPViaNginx(phone, generatedOTP);
    ...
    console.error('[Checkout Step 1 API] Error:', error);
    return NextResponse.json(
